...
export default function Home() {
  ...
  return <main>
    <header className="site-header"><a className="brand" href="#top"><span>Cinci</span><strong>360</strong></a><button className="menu-button" aria-label="Toggle navigation" onClick={()=>setMenu(!menu)}><span/><span/></button><nav className={menu?"open":""}>{["Services","Projects","Team","Reviews"].map(x=><a key={x} href={`#${x.toLowerCase()}`} onClick={()=>setMenu(false)}>{x}</a>)}<a href="/Cinci360-Capability-Statement.pdf" target="_blank">Capability statement ↗</a></nav><a className="header-cta" href="#contact">Start a project</a></header>
    ...
    <section id="projects" className="section projects"><div className="project-heading"><div><p className="eyebrow">Selected work</p><h2>Places worth<br/>understanding.</h2></div><p>From historic landmarks and stadiums to dealerships, manufacturing plants and everyday commercial spaces.</p></div><div className="project-grid">{projects.map((p,i)=><article className={`project-card p${i}`} key={p[0]}><Image src={p[2]} alt={p[0]} fill sizes="(max-width:800px) 100vw, 50vw"/><div><span>{p[1]}</span><h3>{p[0]}</h3><b>View project ↗</b></div></article>)}</div></section>
    ...
    <section id="reviews" className="reviews"><div className="quote">“</div><blockquote>“{reviews[review].quote}”</blockquote><div className="review-meta"><span aria-label="5 out of 5 stars">★★★★★</span><p>{reviews[review].name} · Google review</p></div><div className="review-nav"><button type="button" aria-label="Previous review" onClick={()=>setReview((review-1+reviews.length)%reviews.length)}>←</button><span>{String(review+1).padStart(2,"0")} / {String(reviews.length).padStart(2,"0")}</span><button type="button" aria-label="Next review" onClick={()=>setReview((review+1)%reviews.length)}>→</button></div><a className="google-reviews-link" href="https://maps.app.goo.gl/mYDW7y2nWVpwUXXT9Yeah" target="_blank" rel="noreferrer">See all reviews on Google Maps ↗</a></section>
